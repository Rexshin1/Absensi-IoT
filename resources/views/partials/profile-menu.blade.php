<div class="relative" @click.outside="profileOpen = false">
    ...
    <div x-cloak x-show="profileOpen" x-transition class="absolute right-0 z-50 mt-3 w-44 rounded-sm bg-background p-2 shadow-md dark:bg-darkgray">
        ...
        <form action="{{ route('logout') }}" method="POST" class="p-3 pt-0">
            @csrf
            <button type="submit" class="mt-2 block w-full rounded-md border border-defaultBorder px-3 py-2 text-center text-sm hover:bg-lighterror hover:text-error hover:border-error/30 transition-colors">Logout</button>
        </form>
    </div>
</div>
